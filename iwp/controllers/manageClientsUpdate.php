					...
					elseif($PTC == 'plugins' || $PTC == 'themes'){
						...
						foreach($PTCResponse['upgraded'] as $name => $success){
							...
							elseif(!empty($success)){
								...
								$status = 'error';
								DB::update("?:history_additional_data", array('status' => $status, 'error' => 'unknown', 'errorMsg' => 'An Unknown error occurred.', 'error' => 'unknown_error_occurred_updateall_res_proc'), "historyID='".$historyID."' AND (uniqueName='".$name."' OR MD5(uniqueName)='".$name."') AND detailedAction='".$itemType."'");
							}
						}
					}
					elseif( $PTC == 'translations'){
						...
						if(trim($PTCResponse['upgraded']) == 'updated'){
							$historyAdditionalUpdateData['status'] = 'success';
							DB::update("?:history_additional_data", $historyAdditionalUpdateData, "historyID='".$historyID."' AND uniqueName = 'translations'");
						}
					}
				}
			}
