...
class panelRequestManager{
	...
	public static function changeToUTF8($obj){
		if(is_string($obj)){
			if (function_exists('mb_detect_encoding') ) { 
				if( @mb_detect_encoding($obj, 'UTF-8', true) === false) $obj = utf8_encode($obj); 
			}else{
				// $obj = utf8_encode($obj);
			}
		}elseif( is_array($obj) ){
			foreach($obj as $key=>$value){
				$obj[$key]=self::changeToUTF8($obj[$key]);
			}
	...
	public static function getActionStatus($actionID, $action=''){
		...
		$totalErrorRequest = DB::getField("?:history", "count(status)", "actionID = '".$actionID."' AND status IN ('error','netError' )" );
		...
		$historyAdditionalDatas = DB::getArray("?:history_additional_data HAD, ?:history H", "HAD.*, H.siteID, H.URL, H.microtimeInitiated,H.status AS mainStatus", "H.actionID = '".$actionID."' AND HAD.historyID = H.historyID");		
	...
	public static function getHistoryPageHTML($args){
		...
		if(!empty($args['getKeyword'])){
			$searching = true;
			$keyword = "'".implode("','", explode(',', $args['getKeyword'] ) )."'" ;
	...
	public static function getClientUpdateAvailableSiteIDs(){
		...
		foreach($rawSiteStats as $siteID => $statsArray){			
			$stats = $statsArray['stats'];
			//check iwp-client plugin have any updates
			if(version_compare($stats['client_version'], $clientUpdates['clientUpdateVersion']) == -1  && DB::getExists("?:sites", "siteID", "siteID = '".$siteID."' AND (network = 0 OR (network = 1 AND parent = 1)) AND connectionStatus = 1")){
	...
	public function isAddonSuiteLimitExceededAttempt() {
		return(json_encode(Reg::get('addonSuiteLimitExceededAttemp')));
	}
	
	public function getAddonSuiteMiniLimit() {
	...
	public function getIDForAddonSuite() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
	...
	public function getIDToBeUpgradedFromMini() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
	...
	public function getPriceForSuiteUpgradedFromMini() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
		
		return($suiteDetails['priceForSuiteUpgradedFromMini']);		
	}	
	
	public function getPriceForAddonSuite() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
		
		return($suiteDetails['priceForAddonSuite']);		
	}
	
	public function isAddonSuiteMiniCancelMessage() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
	...
	public function getAddonSuiteMiniActivity() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
	...
	public function checkIsMiniExpired() {
		$suiteDetails = unserialize(getOption('suiteDetails'));
		...
		if($suiteDetails['addonSuiteOrMiniPurchased']=='addonSuiteMini' && $suiteDetails['addonSuiteMiniActivity']=='installed') {
			foreach($installedAddons as  $addon){ 
				if($addon['slug']!='multiUser' && $addon['isValidityExpired']) return(1);
			}
		}
		return(0);
	}
	
	public function checkIsAddonSuiteMiniLimitExceeded($mode='') { 
	...
	public static function clearHistoryTasks($params){
    	if(isset($params) && isset($params['clearWhat'])){
    		if($params['clearWhat'] == 'uncomplete'){
				...
				$historyData = DB::getArray("?:history", "historyID", "status NOT IN ('completed','error','netError') AND ".$accessUsers." showUser = 'Y' "); 
				$error = 'task_cleared';
				...
				if(!empty($params['time'])){
	    			...
	    			$accessUsers = " ";
					setHook('historyHTML', $accessUsers);
					if($accessUsers != " ") $accessUsers = " AND ".$accessUsers;
